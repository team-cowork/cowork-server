...
import com.cowork.roadmap.domain.node.service.impl.QueryRoadmapNodeServiceImpl;
...
class QueryRoadmapNodeServiceTest {
    ...
    private final RoadmapNodeLookupSupport nodeLookupSupport = new RoadmapNodeLookupSupport(nodeRepository,
            referenceRepository);
    private final QueryRoadmapNodeServiceImpl queryRoadmapNodeService = new QueryRoadmapNodeServiceImpl(accessGuard,
            roadmapLookupSupport,
            nodeLookupSupport);

    @Test
    void queryNode_returnsNodeWithReferences() {
        ...
        StepVerifier.create(queryRoadmapNodeService.execute(1L, "MEMBER", 5L)).assertNext(response -> {
    ...
    @Test
    void queryNode_nodeNotFound_failsWithNotFound() {
        when(nodeRepository.findById(99L)).thenReturn(Mono.empty());

        StepVerifier.create(queryRoadmapNodeService.execute(1L, "MEMBER", 99L))
    ...
    private static RoadmapNode node(Long id, Long roadmapId) {
        return RoadmapNode.builder().id(id).roadmapId(roadmapId).position(0).build();
    }
