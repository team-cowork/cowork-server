...
func TestParseRoomName_레거시_포맷을_파싱한다(t *testing.T) {
	t.Parallel()

	parsed, ok := ParseRoomName("voice-123")
	if !ok {
	...
	if parsed.SessionID != "" {
		t.Fatalf("SessionID = %q, want empty", parsed.SessionID)
	}
	if parsed.Format != RoomNameFormatLegacy {
		t.Fatalf("Format = %q, want %q", parsed.Format, RoomNameFormatLegacy)
	}
}
