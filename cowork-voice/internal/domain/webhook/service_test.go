...
func TestHandleEvent_참가_이벤트_첫_입장이면_세션시작과_유저입장을_발행한다(t *testing.T) {
	...
	resolver := &stubLegacySessionResolver{}
	svc := NewWebhookService(repo, resolver, publisher)
...
func TestHandleEvent_구_포맷_참가_이벤트도_활성_세션으로_처리한다(t *testing.T) {
	t.Parallel()
	...
	resolver := &stubLegacySessionResolver{
		resolveResult: &roomdomain.VoiceSession{
			SessionID: "session-1",
			ChannelID: 123,
			TeamID:    456,
			RoomName:  "voice-123",
	...
	svc.HandleEvent(context.Background(), &livekit.WebhookEvent{
		Event:       "participant_joined",
		Room:        &livekit.Room{Name: "voice-123"},
		Participant: &livekit.ParticipantInfo{Identity: "42"},
	})

	if resolver.resolveReq.EventType != EventParticipantJoined {
		t.Fatalf("Resolve() event_type = %q, want %q", resolver.resolveReq.EventType, EventParticipantJoined)
	}
	if resolver.resolveReq.RoomName != "voice-123" {
		t.Fatalf("Resolve() room_name = %q, want voice-123", resolver.resolveReq.RoomName)
	}
	if len(publisher.messages) != 2 {
		t.Fatalf("published messages = %d, want 2", len(publisher.messages))
	}
...
func TestHandleEvent_구_포맷_퇴장_이벤트는_최신_세션으로_fallback한다(t *testing.T) {
	t.Parallel()
	...
	if resolver.resolveReq.EventType != EventParticipantLeft {
		t.Fatalf("Resolve() event_type = %q, want %q", resolver.resolveReq.EventType, EventParticipantLeft)
	}
	if resolver.resolveReq.RoomName != "voice-123" {
		t.Fatalf("Resolve() room_name = %q, want voice-123", resolver.resolveReq.RoomName)
	}
...
func TestHandleEvent_구_포맷_룸종료도_활성_세션을_종료한다(t *testing.T) {
	t.Parallel()
	...
	if resolver.resolveReq.EventType != EventRoomFinished {
		t.Fatalf("Resolve() event_type = %q, want %q", resolver.resolveReq.EventType, EventRoomFinished)
	}
	if resolver.resolveReq.RoomName != "voice-123" {
		t.Fatalf("Resolve() room_name = %q, want voice-123", resolver.resolveReq.RoomName)
	}
...
func (s *stubSessionRepository) EndSession(_ context.Context, sessionID string, _ time.Time) error {
...
type stubLegacySessionResolver struct {
	resolveReq    LegacySessionResolveRequest
	resolveResult *roomdomain.VoiceSession
	resolveErr    error
}

func (s *stubLegacySessionResolver) Resolve(_ context.Context, req LegacySessionResolveRequest) (*roomdomain.VoiceSession, error) {
	s.resolveReq = req
