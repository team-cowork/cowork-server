...
func TestHandleEvent_구_포맷_참가_이벤트도_활성_세션으로_처리한다(t *testing.T) {
	...
	repo := &stubRepository{
		findActiveSessionResult: &roomdomain.VoiceSession{
			SessionID: "session-1",
			ChannelID: 123,
			TeamID:    456,
			RoomName:  "voice-123",
	...
	if repo.findActiveSessionChannelID != 123 {
		t.Fatalf("FindActiveSession() channelID = %d, want 123", repo.findActiveSessionChannelID)
	}
	if len(publisher.messages) != 2 {
		t.Fatalf("published messages = %d, want 2", len(publisher.messages))
	}
...
func TestHandleEvent_구_포맷_퇴장_이벤트는_최신_세션으로_fallback한다(t *testing.T) {
	...
	joinedAt := time.Unix(1700000000, 0).UTC()
	repo := &stubRepository{
		findLatestSessionByChannelResult: &roomdomain.VoiceSession{
			SessionID: "session-1",
			ChannelID: 123,
			TeamID:    456,
			RoomName:  "voice-123",
	...
	if repo.findLatestSessionByChannelID != 123 {
		t.Fatalf("FindLatestSessionByChannel() channelID = %d, want 123", repo.findLatestSessionByChannelID)
	}
	if len(publisher.messages) != 1 {
		t.Fatalf("published messages = %d, want 1", len(publisher.messages))
	}
...
type stubRepository struct {
	...
	findActiveSessionChannelID       int64
	findLatestSessionByChannelResult *roomdomain.VoiceSession
	findLatestSessionByChannelErr    error
	findLatestSessionByChannelID     int64
...
func (s *stubRepository) FindActiveSession(_ context.Context, channelID int64) (*roomdomain.VoiceSession, error) {
	s.findActiveSessionChannelID = channelID
	return s.findActiveSessionResult, s.findActiveSessionErr
}

func (s *stubRepository) FindLatestSessionByChannel(_ context.Context, channelID int64) (*roomdomain.VoiceSession, error) {
	s.findLatestSessionByChannelID = channelID
	return s.findLatestSessionByChannelResult, s.findLatestSessionByChannelErr
}
