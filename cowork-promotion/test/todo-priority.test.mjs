...
describe('parsePriority', () => {
  for (const { label, marker, priority } of [
    ...
    it(`classifies ${label} TODO priority with or without its marker`, () => {
      assert.equal(parsePriority(label), priority);
      assert.equal(parsePriority(`${marker} ${label}`), priority);
    });
  }

  it('treats missing or unsupported TODO priorities as unspecified', () => {
...
describe('sortTodoItems', () => {
  it('orders TODOs by high, medium, low, then unspecified priority before registry order', () => {
    const items = [
  ...
  it('places missing and unsupported priorities after all known priorities', () => {
    const items = [
  ...
  it('uses registry order for TODOs with the same priority', () => {
    const items = [
