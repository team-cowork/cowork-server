...
- 팀 사용자 정의 역할과 채널 사이의 `message_read` 허용/거부 정책
...
역할에 종속되는 리소스 설정은 `cowork-preference` 소유로 유지합니다. 채널 역할 정책은
`(team_id, channel_id, role_id)`별 JSONB full state이며, 현재 허용하는 키는 boolean `message_read` 하나입니다.
역할의 높낮이는 `tb_team_role_definitions.priority`로 표현하고 동일 우선순위에서 충돌하면 deny(`false`)를 우선합니다.
...
변경을 요청하는 유일한 진입 경로입니다. 채널 역할 정책도 동일하게 `cowork-channel`이 공개 API와 채널 관리 권한 검증을
담당하고 이 서비스에는 직접 HTTP mutation API를 두지 않습니다.

## 이벤트와 의존성

- Kafka consume: `team.member.event`, `preference.team-role.command`, `preference.github-repo.setting.command`,
  `preference.channel-role-policy.command`
- Kafka produce: `preference.status.changed`, `preference.team.setting.changed`,
  ...
  `preference.github-repo.setting.result`, `preference.channel-role-policy.changed`,
...
`preference.github-repo.setting.state`, `preference.channel-role-policy.changed`에는 시작 시점과 주기적으로
full-state snapshot과 partition completion marker를
...
채널 역할 정책은 upsert와 tombstone SQL의 `GREATEST` 비교로 같은 단조 증가 규칙을 보장합니다. mutation event와 주기
snapshot이 모두 이 값을 `occurredAt`으로 사용합니다. 채널 알림 GET은 동시 PUT 뒤 오래된
...
없습니다. 역할·할당 DELETE와 멤버 탈퇴 fence도 PostgreSQL에 영구 보존합니다. ASSIGN은 같은 할당 tombstone을 제거하고
멤버 fence보다 엄격히 새로운 DB 상태 버전을 사용합니다. 따라서 `preference.team-role.changed` Kafka topic generation이
바뀌어도 full snapshot이 active row와 모든 tombstone/fence를 함께 재생해 과거 권한을 되살리지 않습니다. snapshot completion
marker는 upstream `team.member.event` projection readiness가 현재 generation을 따라잡은 뒤에만 발행합니다.
GitHub 저장소 설정 `UPDATE` command도 같은 방식으로 설정 mutation, state/result outbox, inbox를 원자적으로
...
채널 역할 정책 command도 durable inbox와 `team.member.event` projection을 사용합니다. actor 멤버십의 active 상태와
version, `role_id`의 동일 팀 소속을 확인한 뒤 정책 mutation, full-state event, terminal result, inbox를 한 트랜잭션으로
커밋합니다. DELETE는 compacted topic의 과거 UPSERT가 snapshot 복구 과정에서 되살아나지 않도록 영구 tombstone을 남깁니다.
역할 또는 팀 삭제도 FK row를 제거하기 전에 관련 정책 DELETE tombstone을 같은 트랜잭션에 적재합니다.
...
`KAFKA_GROUP_ID_GITHUB_REPO_SETTING_COMMAND`, `KAFKA_GROUP_ID_CHANNEL_ROLE_POLICY_COMMAND`, `EUREKA_*` 직접 환경변수는
중앙 설정보다 높은 우선순위로 적용됩니다.
