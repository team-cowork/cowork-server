...
class PreferenceTeamRoleChangedConsumer(
    ...
    fun consume(record: ConsumerRecord<String, String>) {
        ...
        val event = runCatching { objectMapper.readValue(record.value(), PreferenceTeamRoleChangedEvent::class.java) }
            .getOrElse {
        ...
        return when (event.eventType) {
            "ROLE_UPSERTED" -> when {
                ...
                event.name.isNullOrBlank() ||
                    ...
                    event.permissions == null -> "ROLE_UPSERTED read model이 완전하지 않습니다."
                else -> null
            }
            "ROLE_DELETED" -> when {
            ...
            "MEMBER_ASSIGNMENTS_DELETED" -> when {
                event.accountId == null || event.accountId <= 0 -> "멤버 tombstone에는 유효한 accountId가 필요합니다."
