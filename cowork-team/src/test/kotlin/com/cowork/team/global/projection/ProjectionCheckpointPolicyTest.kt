...
class ProjectionCheckpointPolicyTest {
    ...
    @Test
    fun `모든 partition이 고정 barrier에 도달해야 role과 assignment aggregate가 ready다`() {
        val barriers = mapOf(stream to mapOf(0 to barrier(2), 1 to barrier(4)))
        val behind = mapOf(stream to mapOf(0 to checkpoint(2), 1 to checkpoint(3)))
        val caughtUp = mapOf(stream to mapOf(0 to checkpoint(2), 1 to checkpoint(4)))
        assertFalse(ProjectionCatchUpPolicy.isReady(setOf(stream), barriers, behind))
        assertTrue(ProjectionCatchUpPolicy.isReady(setOf(stream), barriers, caughtUp))
    }
    ...
    fun `유효한 snapshot completion marker는 marker와 next offset을 같은 처리 경로에서 저장한다`() {
        ...
        val record = ConsumerRecord(
            ...
            """{"eventType":"PROJECTION_SNAPSHOT_COMPLETED","topic":"topic","partition":1,"snapshotId":"00000000-0000-0000-0000-000000000001","occurredAt":"2026-08-26T00:00:00Z","source":"cowork-preference"}""",
        )
