...
class TeamMemberEventConsumerTest {
    ...
    private val handler = mockk<ChannelLifecycleHandler>(relaxed = true)
    ...
    fun `복합 key와 occurredAt이 유효한 member event만 반영한다`() {
        ...
        consumer.consume(ConsumerRecord(Topics.TEAM_MEMBER_EVENT, 0, 1L, "100:7", "{}"))
        consumer.consume(ConsumerRecord(Topics.TEAM_MEMBER_EVENT, 0, 2L, "100:8", "{}"))
