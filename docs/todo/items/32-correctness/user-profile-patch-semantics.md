...
> **2026-09-10 완료:** 미제공 필드와 역할을 변경 대상에서 제외하고, 빈 객체는 이벤트를 만들지 않는 no-op으로 처리한다. 명시적 `null`, 빈 역할 배열, 잘못된 역할 값을 구분하는 정책과 OpenAPI 계약을 반영했으며 `cowork-user` 정책 단위 테스트 47개가 통과했다.
