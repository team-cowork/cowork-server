...
`ChannelEventPublisher`는 채널 삭제 transaction에서 `channel.event.v2`에 `DELETED` full state를 transactional outbox로
발행한다. 삭제 ledger와 주기 snapshot, completion marker도 유지된다. 그러나 `cowork-preference`의 `KafkaConfig`와
`MainVerticle`에는 `channel.event.v2` consumer가 없고 PostgreSQL에도 채널 수명주기 projection이나 영구 삭제 fence가 없다.
...
- `cowork-config`와 `AppConfig`에 `channel.event.v2` topic과 전용 consumer group 설정을 추가한다.
- `channel.event.v2` parser와 Vert.x consumer를 checkpoint, topic UUID, snapshot completion marker, invalid-record latch 규칙에 맞춰 구현한다.
...
- `cowork-preference`의 Eureka 등록과 policy command readiness가 필요한 `channel.event.v2` generation과 fresh snapshot을 반영하게 한다.
...
- cold start에서 `channel.event.v2`와 `preference.channel-role-policy.changed`가 순환 대기 없이 각각 fresh snapshot marker 뒤 준비되는지 검증한다.
