...
- 계정 A의 토큰으로 계정 B의 설정과 알림 설정을 조회·수정할 수 없는지 통합 테스트한다.
- 일반 멤버가 팀·프로젝트·채널 설정과 프로젝트 역할을 변경할 수 없는지 검증한다.
- 권한 보유자와 전역 ADMIN의 허용 범위를 정책대로 검증한다.
- 멤버십 삭제 이벤트가 반영된 뒤 기존 사용자의 접근이 거부되는지 검증한다.
- 공개 Preference 경로 전체에 요청자 컨텍스트 검증이 빠짐없이 적용됐는지 라우트 테스트로 확인한다.
...
- 허용·거부 시나리오가 HTTP 통합 테스트로 자동화되어 있다.
