...
>
> **2026-08-30 평가 규칙 확정:** 기존 `TeamRoleDefinition.priority`의 큰 숫자를 상위로 사용한다. 권한 키가 없으면 다음
> 낮은 priority로 상속하고, 같은 priority의 값이 충돌하면 `false`를 적용하며, 끝까지 미지정이면 거부한다. built-in
> `OWNER`만 허용하되 비공개 채널의 활성 멤버십 조건은 우회하지 않는다.
...
따라서 역할 정의의 팀 전역 문자열 권한이 아니라 역할과 채널을 연결하는 별도 정책 aggregate가 필요하다. 권한 키가 없으면
즉시 읽기를 거부하는 정책을 먼저 활성화하면 기존 정상 멤버 전체가 차단될 수 있으므로, 키 미지정 시 상속 규칙과 기존 데이터
backfill을 함께 정한다. 반대로 기존 멤버십 검사만 유지하면 `message_read=false`인 역할도 채널 메타데이터, 저장 메시지,
실시간 메시지와 알림 본문을 계속 받을 수 있다. 로컬 projection이 준비된 뒤 모든 읽기 경로에 같은 effective policy 판정을
적용한다.
...
|------|---------------------|-------------|
| 권한 어휘 | 검증 없는 문자열 집합 | 바인딩 JSON의 첫 읽기 권한을 `message_read`로 정의하고 추가 키 catalog를 확정한다 |
...
| 키 미지정 | 바인딩과 기본값이 없음 | 다음 낮은 priority로 상속하고 끝까지 미지정이면 거부한다 |
...
- `role × channel`별 `message_read` boolean full state와 삭제 tombstone을 `cowork-preference`가 소유하고 compacted
  state event로 발행한다.
- 같은 priority의 충돌은 deny 우선, 상위 role 미지정은 다음 낮은 priority로 상속, 끝까지 미지정은 deny로 구현했다.
...
- 비동기 정책 변경 API와 operation 조회, command inbox/outbox, projection readiness fail-closed를 추가했다.
...
- ~~`message_read`를 포함한 허용 권한 이름과 boolean 값 검증을 추가하고 알 수 없는 키를 거부한다.~~
...
- 권한 어휘에 없는 문자열과 잘못된 JSON 형태가 저장되거나 이벤트로 발행되지 않는지 검증한다.
...
- 허용 권한 이름, 합성 규칙, built-in 기본값과 기존 데이터 전환 정책이 코드와 계약 문서에 일치한다.
