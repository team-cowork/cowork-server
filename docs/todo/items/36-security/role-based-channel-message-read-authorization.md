...
- 알 수 없는 권한 키 제거, 알려진 키 타입 검증, 누락된 `message_read=false` 기본값을 정책 단위 테스트로 검증한다.
- 정책 바인딩 부재의 하위 역할 상속과 명시적인 빈 바인딩의 deny 판정을 단위 테스트로 검증한다.
- built-in 역할, 역할이 없는 멤버, 여러 사용자 정의 역할 조합의 effective permission을 표 기반 테스트로 검증한다.
- 공개·비공개·프로젝트 채널의 허용·거부·멤버십 회수 조합과 민감 데이터 비노출 판단을 access service 단위 테스트로 검증한다.
- canonical request hash, event payload, tombstone, snapshot replay, WebSocket 전달은 정적 검토와 운영 지표 대상으로 두며 자동화 기술 테스트를 추가하지 않는다.
