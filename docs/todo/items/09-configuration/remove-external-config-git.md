...
`overrides`에 등록된 flat 키는 Spring 클라이언트의 placeholder 해석 소스가 되는 동시에, 비Spring 클라이언트가 키 이름으로 직접 읽는 값이 된다. 현재 등록 대상은 `S3_INTERNAL_ENDPOINT`, `S3_PUBLIC_ENDPOINT`, `S3_PUBLIC_BASE_URL`, `LIVEKIT_URL`, `LIVEKIT_WS_URL`, `PUBLIC_WEB_ORIGINS`, `PUBLIC_API_BASE_URL`, `GITHUB_APP_SERVICE_URL`이다.
...
`cowork-config`의 `local`·`prod` 프로파일은 모두 `Vault + classpath native` 조합이다. 11개 Config
application의 `*-local.yml`과 `*-prod.yml`이 저장소에 있으며, Config Server와 Compose에서
`CONFIG_GIT_*` 및 외부 Git backend는 제거됐다.

저장소 밖의 과거 Config Git 내용과 실제 배포 인스턴스 환경 파일은 이 코드베이스만으로 확인할 수
없다. 따라서 구현을 되돌리는 것이 아니라, 외부 원본에만 있던 key가 누락되지 않았는지 비교하고
staging에서 기동을 검증한 뒤 기존 저장소와 자격 증명을 폐기하는 작업이 남아 있다.
...
다음 항목은 기존 Config Git과 자격 증명을 폐기하기 전에 확인한다.

- 과거 배포 기록의 `CONFIG_GIT_URI`가 가리킨 저장소와 실제 운영 label/branch를 확인한다.
...
- 공개 URL과 보류된 GitHub App service URL을 분리하고, 실제 외부 네트워크 경로로 도달 가능한지 확인한다.
...
1. [완료] 각 key를 native 일반 설정, Vault 시크릿, 공유 topology 값, 인스턴스별 client 환경변수로 분류한다.
2. [완료] 11개 `cowork-*-prod.yml`과 필요한 prod 공통 설정을 추가한다.
3. [완료] 각 런타임의 환경변수 우선순위와 placeholder 처리 차이를 보완한다.
4. [완료] Config Server `prod`를 native backend로 교체하고 저장소의 `CONFIG_GIT_*` 참조를 제거한다.
5. [남음] 외부 Config Git과 운영 env의 property key를 값 없이 inventory하고 native 응답과 비교한다.
6. [남음] staging 또는 별도 검증 환경에서 런타임별 기동을 확인한다.
7. [남음] 검증 후 외부 Config Git 저장소와 관련 자격 증명을 폐기한다.
...
- `deploy/prod/services/config.sh`
...
- 운영 Config Server 배포 스크립트가 `CONFIG_GIT_*` 없이 Vault와 native 설정을 사용하도록 구성되어 있는지 정적으로 확인한다.
