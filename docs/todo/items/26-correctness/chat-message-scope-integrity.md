...
- 프로젝트 채널에서 `projectId`를 생략하거나 다른 값으로 보내도 잘못된 범위의 메시지가 저장되지 않는지 검증한다.
- 팀 채널과 DM 채널 메시지의 `teamId`·`projectId`가 서버 projection 계약대로 저장되는지 검증한다.
- 다른 채널의 유효한 ObjectId를 `parentMessageId`로 보내면 요청이 거부되는지 검증한다.
- 메시지 목록의 `mentionedMessage`와 알림의 부모 작성자 조회가 같은 채널 밖의 메시지를 반환하지 않는지 검증한다.
- 채널 삭제 또는 멤버십 회수와 메시지 발행이 경합할 때 잘못된 범위의 Kafka 이벤트가 생성되지 않는지 검증한다.
...
- 채널·프로젝트·부모 범위 불변식이 API와 Kafka 소비 양쪽의 자동 테스트로 보호되어 있다.
