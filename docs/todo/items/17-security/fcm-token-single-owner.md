...
- 서로 다른 두 계정이 같은 token을 순차·동시에 등록해도 최종 행이 하나인지 MySQL 통합 테스트한다.
- 소유권 이전 후 이전 계정 대상 알림에 해당 token이 포함되지 않는지 검증한다.
- 같은 계정의 재등록이 멱등하게 동작하는지 확인한다.
- 기존 중복 데이터 정리와 unique 제약 추가가 실제 데이터 사본에서 성공하는지 검증한다.
...
- 단일 소유권과 동시 등록 경합이 통합 테스트로 보장되어 있다.
