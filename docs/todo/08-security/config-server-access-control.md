...
Config Server는 Vault에서 조회한 데이터베이스 비밀번호, JWT·세션 서명키, OAuth 자격 증명, MinIO 자격 증명 등을 각 서비스에 전달한다. 따라서 포트에 도달할 수 있는 사용자가 `/{application}/{profile}` 형식의 Config API로 다른 서비스의 설정과 시크릿을 조회할 수 있다.
