...
`cowork-chat`의 실시간 메시지 파이프라인(WebSocket ↔ Kafka ↔ MongoDB)을 대상으로 진행한 성능 병목 분석 기록입니다. 분석 자체(2026-09-17 기준)는 코드 변경 없이 실제 코드·호출 흐름·쿼리 구조만 근거로 작성했으며, 이후 3번 항목은 실제로 수정을 적용해 아래에 상태를 표시했다. 이후 코드가 더 바뀌면 이 문서의 위치·줄 번호는 최신 상태와 다를 수 있습니다.
...
채널 단위로 판정에 필요한 원시 데이터(채널 정보, 멤버, 팀 역할 할당, 역할 정책)를 짧은 캐시(채널별 in-memory 또는 Redis, TTL 수 초~수십 초)로 보관하고, 이미 이 데이터를 변경하는 컨슈머(`ChannelEventConsumer`, `MembershipConsumer`, `TeamRoleEventConsumer`, `ChannelRolePolicyEventConsumer`, `TeamMemberEventConsumer`)가 처리될 때 해당 채널/팀 캐시를 무효화한다. 이 컨슈머들은 이미 `evictUnauthorizedSockets()`를 호출하는 지점이 있으므로, 같은 지점에 무효화 훅을 추가하는 것이 자연스럽다.

**기대 효과**: 캐시 히트 시 Mongo 왕복 4회 → 0회, 메시지 브로드캐스트 지연시간 감소, 메시지량 증가에 선형 비례하던 DB 부하 제거.

**Trade-off**: 캐시 무효화가 실제 이벤트 처리보다 늦으면 짧은 시간 오래된 권한으로 판정될 위험이 있다(기존 `evictUnauthorizedSockets` 호출과 같은 지점에서 무효화하면 최소화 가능). 활성 채널 수 × 채널당 멤버 수 규모의 캐시 메모리가 추가된다.
...
- 위 표의 우선순위대로 착수 여부를 정한다. 3번은 리스크가 가장 낮아 먼저 적용해볼 수 있다.
- 1번(접근 제어 캐싱)은 캐시 무효화 정확성이 핵심이므로, 실제 구현 시 `evictUnauthorizedSockets` 호출 경로와 무효화 지점을 함께 검토해야 한다.
