...
`editHistory`는 수정 직전 내용을 배열 뒤에 추가하며 최근 20건만 보관합니다. 이 상한은 이후 수정에서만 적용되며, 이미 20건을 넘긴 기존 문서는 별도 일괄 정리 없이 다음 수정이 한 번 더 일어나야 잘립니다.
...
replica가 공유하는 단일 도큐먼트로, 마지막 전체 재구축 결과와 레거시 백필 완료 여부를 남깁니다.
