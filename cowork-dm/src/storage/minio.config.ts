...
export function buildMinioConfig(configService: ConfigService): MinioConfig {
    ...
    return {
        ...
        maxFileSizeBytes: Number(getOptionalConfig(
            configService,
            ['minio.dmMaxFileSizeBytes', 'MINIO_DM_MAX_FILE_SIZE_BYTES', 'MINIO_CHAT_MAX_FILE_SIZE_BYTES'],
        ) ?? DEFAULT_MAX_FILE_SIZE_BYTES),
        allowedContentTypes: (
            getOptionalConfig(configService, ['minio.dmAllowedContentTypes', 'MINIO_DM_ALLOWED_CONTENT_TYPES', 'MINIO_CHAT_ALLOWED_CONTENT_TYPES'])
            ?? [
                'video/mp4',
                'video/quicktime',
                'video/x-msvideo',
        ...
        uploadRateLimitWindowMs: Number(getOptionalConfig(
            configService,
            ['minio.dmUploadRateLimitWindowMs', 'MINIO_DM_UPLOAD_RATE_LIMIT_WINDOW_MS'],
        ) ?? DEFAULT_UPLOAD_RATE_LIMIT_WINDOW_MS),
        uploadRateLimitMaxRequests: Number(getOptionalConfig(
            configService,
            ['minio.dmUploadRateLimitMaxRequests', 'MINIO_DM_UPLOAD_RATE_LIMIT_MAX_REQUESTS'],
