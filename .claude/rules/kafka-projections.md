...
- Coordinate every producer, snapshot marker, consumer, config/default/override, launch script, and provisioning reference. Launch-script environment defaults can override Config Server values, so include `scripts/run/**` in the reference audit. Reuse producer topic constants for completion markers. For the four v2 streams, follow `docs/kafka-state-topic-cutover.md`.
