...
- Coordinate every producer, snapshot marker, consumer, config/default/override, and provisioning reference. Reuse producer topic constants for completion markers. For the four v2 streams, follow `docs/kafka-state-topic-cutover.md`.
- This repository's v2 cutover uses a maintenance window: stop old writers/consumers, account for pending outbox rows, create the new topics, rebuild the affected projection data and checkpoints, and wait for full snapshots and catch-up before reopening traffic. Rolling versions against the same projection store or copying raw old records into the new topic is not a migration strategy. A zero-downtime migration needs a separately designed dual-write/backfill and isolated consumer store.
