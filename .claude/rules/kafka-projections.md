...
- A compacted state topic's message key is permanent schema, not an implementation detail. Compaction retains the latest record per key indefinitely, so records written under a retired key format never expire and every replay from the earliest offset keeps reading them. Never change a key format in place. A breaking key change requires a new topic version that producers and every consumer move to together, after which the previous topic is retired and its name is never reused. Backward-compatible parsing on the consumer is a stopgap that keeps replay working until that cutover, never the resolution.
