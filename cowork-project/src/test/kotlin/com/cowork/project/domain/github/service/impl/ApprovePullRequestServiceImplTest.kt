...
class ApprovePullRequestServiceImplTest :
    DescribeSpec({
        ...
        beforeEach {
            ...
            service = ApprovePullRequestServiceImpl(repoAccessResolver, usernameResolver, commandPublisher)

            every { commandPublisher.publishPullRequestApprove(any()) } returns Unit
        }

        describe("ApprovePullRequestServiceImpl 클래스의") {
            describe("execute 메서드는") {
                context("수정 권한이 있고 GitHub 계정이 연동된 경우") {
                    it("수정 권한으로 레포를 해석하고 githubUsername을 담아 승인 커맨드를 발행한다") {
